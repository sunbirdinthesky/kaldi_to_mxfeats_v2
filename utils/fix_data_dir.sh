...
function filter_file {
  ...
  ./process_env/utils/filter_scp.pl $filter ${file_to_filter}.tmp > $file_to_filter
...
function filter_recordings {
  ...
  # recording-ids.
    ...
    ./process_env/utils/filter_scp.pl $data/wav.scp $tmpdir/recordings > $tmpdir/recordings.tmp
...
function filter_speakers {
  ...
  ./process_env/utils/utt2spk_to_spk2utt.pl $data/utt2spk > $data/spk2utt
  ...
  ./process_env/utils/spk2utt_to_utt2spk.pl $data/spk2utt > $data/utt2spk
...
function filter_utts {
  ...
  for x in feats.scp text segments utt2lang $maybe_wav; do
    if [ -f $data/$x ]; then
      ./process_env/utils/filter_scp.pl $data/$x $tmpdir/utts > $tmpdir/utts.tmp
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text segments utt2lang utt2dur $maybe_wav; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
      if ! cmp -s $data/$x <( ./process_env/utils/filter_scp.pl $tmpdir/utts $data/$x ) ; then
        ./process_env/utils/filter_scp.pl $tmpdir/utts $data/.backup/$x > $data/$x
      fi
    fi
  done
