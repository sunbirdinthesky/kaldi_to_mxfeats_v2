...
. ./process_env/utils/parse_options.sh
...
cat $srcdir/utt2spk | ./process_env/utils/apply_map.pl -f 1 $destdir/utt_map  | \
  ./process_env/utils/apply_map.pl -f 2 $destdir/spk_map >$destdir/utt2spk

./process_env/utils/utt2spk_to_spk2utt.pl <$destdir/utt2spk >$destdir/spk2utt

if [ -f $srcdir/feats.scp ]; then
  ./process_env/utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/feats.scp >$destdir/feats.scp
fi


if [ -f $srcdir/segments ]; then
  ./process_env/utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/segments >$destdir/segments_t
  ./process_env/utils/apply_map.pl -f 2 $destdir/spk_map <$destdir/segments_t >$destdir/segments
  rm $destdir/segments_t
  #cp $srcdir/wav.scp $destdir
  ./process_env/utils/apply_map.pl -f 1 $destdir/spk_map <$srcdir/wav.scp >$destdir/wav.scp
  if [ -f $srcdir/reco2file_and_channel ]; then
    cp $srcdir/reco2file_and_channel $destdir/
  fi
else # no segments->wav indexed by utt.
  if [ -f $srcdir/wav.scp ]; then
    ./process_env/utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/wav.scp >$destdir/wav.scp
  fi
fi

if [ -f $srcdir/text ]; then
  ./process_env/utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/text >$destdir/text
fi
if [ -f $srcdir/utt2dur ]; then
  ./process_env/utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/utt2dur >$destdir/utt2dur
fi
if [ -f $srcdir/spk2gender ]; then
  ./process_env/utils/apply_map.pl -f 1 $destdir/spk_map <$srcdir/spk2gender >$destdir/spk2gender
fi
if [ -f $srcdir/cmvn.scp ]; then
  ./process_env/utils/apply_map.pl -f 1 $destdir/spk_map <$srcdir/cmvn.scp >$destdir/cmvn.scp
fi
...
./process_env/utils/validate_data_dir.sh $validate_opts $destdir
