...
./process_env/utils/validate_data_dir.sh --no-text --no-feats $data || exit 1;
...
for n in $(seq $nj); do
  ...
  ./process_env/utils/create_data_link.pl $fbankdir/raw_fbank_$name.$n.ark
done
...
if [ -f $data/segments ]; then
  ...
  ./process_env/utils/split_scp.pl $data/segments $split_segments || exit 1;
  rm $logdir/.error 2>/dev/null

  $cmd JOB=1:$nj $logdir/make_fbank_${name}.JOB.log \
    ./process_env/bin/extract-segments scp,p:$scp $logdir/segments.JOB ark:- \| \
    ./process_env/bin/compute-fbank-feats $vtln_opts --verbose=2 --config=$fbank_config ark:- ark:- \| \
    ./process_env/bin/copy-feats --compress=$compress ark:- \
  ...
  ./process_env/utils/split_scp.pl $scp $split_scps || exit 1;

  $cmd JOB=1:$nj $logdir/make_fbank_${name}.JOB.log \
    ./process_env/bin/compute-fbank-feats $vtln_opts --verbose=1 --config=$fbank_config scp,p:$logdir/wav.JOB.scp ark:- \| \
    ./process_env/bin/copy-feats --compress=$compress ark:- \
     ark,scp:$fbankdir/raw_fbank_$name.JOB.ark,$fbankdir/raw_fbank_$name.JOB.scp \
     || exit 1;
