...
class PrefetchingIter(DataIter):
    ...
    def __init__(self, iters, rename_data=None, rename_label=None):
        ...
        if rename_data is None:
            ...
            self.provide_data_type = sum([i.provide_data_type for i in iters], [])
        else:
            self.provide_data = sum([[(r[n], s) for n, s in i.provide_data] \
                                    for r, i in zip(rename_data, iters)], [])
            self.provide_data_type = sum([[(r[n], s) for n, s in i.provide_data_type] \
                                    for r, i in zip(rename_data, iters)], [])
        if rename_label is None:
            self.provide_label = sum([i.provide_label for i in iters], [])
            self.provide_label_type = sum([i.provide_label_type for i in iters], [])
        else:
            self.provide_label = sum([[(r[n], s) for n, s in i.provide_label] \
                                    for r, i in zip(rename_label, iters)], [])
            self.provide_label_type = sum([[(r[n], s) for n, s in i.provide_label_type] \
                                    for r, i in zip(rename_label, iters)], [])
...
class SimpleBatch(object):
    def __init__(self, data_names, data, label_names, label, pad, index):
        self.data = data
        self.label = label
        self.data_names = data_names
        self.label_names = label_names
        self.bucket_key = label[0].shape[1]

        self.pad = pad
        self.index = index

        if len(self.label_names)==2: # index as second label
            self.label.append(array(index, dtype=index.dtype))
            print self.label[1].shape, self.label[1].dtype

        self.provide_data = [(n, x.shape) for n, x in zip(self.data_names, self.data)]
        self.provide_data_type = [(n, x.dtype) for n, x in zip(self.data_names, self.data)]
        self.provide_label = [(n, x.shape) for n, x in zip(self.label_names, self.label)]
        self.provide_label_type = [(n, x.dtype) for n, x in zip(self.label_names, self.label)]
...
class MXDataIter(DataIter):
    ...
    """
    def __init__(self, handle, data_names=['data'], label_name='softmax_label', **_):
        ...
        if _.has_key("bucket_sizes"):
            self.default_bucket_key = max(_["bucket_sizes"])
            if _.has_key("index_as_second_label"):
                self.label_names.append(_["index_as_second_label"])
                logging.info("index as second label, name=%s", self.label_names[1])
 
        # load the first batch to get shape information
        self.first_batch = None
        self.first_batch = self.next()
        assert len(data_names) <= len(self.first_batch.data)
        if len(data_names) < len(self.first_batch.data):
            logging.info("use less data slots than iter provided")
            self.num_slots = len(data_names)
        label = self.first_batch.label[0]

        # properties
        self.provide_data = [(data_name, data.shape) 
            for data_name, data in zip(data_names, self.first_batch.data)]
        ...
        if len(self.label_names)==2: # index as second label
            self.provide_label.append((self.label_names[1], (label.shape[0],)))
            self.provide_label_type.append((self.label_names[1], (label.dtype,)))
        self.batch_size = label.shape[0]

        if _.has_key("bucket_sizes"):
            def replace_bucket_key(shape, bucket_key):
                if len(shape) >= 2:
                    return tuple([shape[0]]+[bucket_key]+list(shape[2:]))
                else:
                    return shape
            self.provide_data = [(data_name, replace_bucket_key(data.shape, self.default_bucket_key)) 
                                 for data_name, data in zip(data_names, self.first_batch.data)]
            self.provide_label = [(name, replace_bucket_key(shape, self.default_bucket_key))
                                  for name, shape in self.provide_label]
    ...
    def next(self):
        ...
        if next_res.value:
            if self.default_bucket_key is None:
                ...
                return SimpleBatch(data_names=self.data_names, data=self.getdata(), 
                                   label_names=self.label_names, label=[self.getlabel()], 
                                   pad=self.getpad(), index=self.getindex())
    ...
    def getdata(self):
        if self.num_slots is None:
            num_slots = ctypes.c_int(0)
            check_call(_LIB.MXDataIterGetNumDataSlots(self.handle, ctypes.byref(num_slots)))
            self.num_slots = num_slots.value
        data_list = []
        for i in xrange(self.num_slots):
            hdl = NDArrayHandle()
            check_call(_LIB.MXDataIterGetData(self.handle, mx_uint(i), ctypes.byref(hdl)))
            data_list.append(NDArray(hdl, False))
        return data_list
    
    def get_data_label_slot_num(self):
        data_size = mx_uint() 
        label_size = mx_uint()  
        check_call(_LIB.MXDataIterGetDataAndLabelSize(self.handle, \
                                                      ctypes.byref(data_size),  \
                                                      ctypes.byref(label_size))) 
        return [data_size.value, label_size.value]

    def getdata_by_id(self, id):
        hdl = NDArrayHandle()
        data_id = mx_uint(id)
        check_call(_LIB.MXDataIterGetData(self.handle, data_id, ctypes.byref(hdl)))
