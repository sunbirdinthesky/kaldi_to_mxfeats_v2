...
def create(name='local', hybrid_num_batches_per_block=0):
    ...
    check_call(_LIB.MXKVStoreCreate(c_str(name),
                                    ctypes.byref(handle)))
    if hybrid_num_batches_per_block>0:
        return HybridKVStore(handle, hybrid_num_batches_per_block)
    else:
        return KVStore(handle)

class HybridKVStore(KVStore):
    def __init__(self, handle, num_batches_per_block):
        super(HybridKVStore, self).__init__(handle)
        self._index_update_count = {}
        self.remote_pull = {}
        self.param_dict = {}
        self.param_backup = {}
        self.states_dict = {}
        self.num_device = None
        self.num_batches_per_block = num_batches_per_block
  
    def set_optimizer(self, optimizer):
        assert isinstance(optimizer, list) and len(optimizer)==2
        self.local_optimizer = optimizer[0]
        super(HybridKVStore, self).set_optimizer(optimizer[1])

    def push(self, key, value, priority=0):
        #local update
        for k, tup in enumerate(zip(value, self.param_dict[key])):
            index = key*self.num_device + k
            (grad, weight) = tup
            if index not in self.states_dict:
                self.states_dict[index] = self.local_optimizer.create_state(index, grad)
            self.local_optimizer.update(index, weight, grad, self.states_dict[index])

        self.remote_pull[key] = self._update_count(key) % self.num_batches_per_block == 0

        if self.remote_pull[key]:
            #store delta in grad arrays
            for delta, weight, weight_backup in zip(
                    value, self.param_dict[key], self.param_backup[key]):
                delta[:] = weight - weight_backup
            super(HybridKVStore, self).push(key, value, priority)

    def pull(self, key, out=None, priority=0):
        if key not in self.param_dict and out is not None:
            value = out
            self.param_dict[key] = value
            self.param_backup[key] = [zeros(weight.shape, weight.context) 
                                      for weight in value]
            self.remote_pull[key] = True
            if self.num_device is None:
                self.num_device = len(value)
            else:
                assert self.num_device == len(value)
        if self.remote_pull[key]:
            super(HybridKVStore, self).pull(key, out, priority)
            for weight, weight_backup in zip(out, self.param_backup[key]):
                weight.copyto(weight_backup)
