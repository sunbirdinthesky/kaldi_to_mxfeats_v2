...
class BMUF(Optimizer):
    ...
    is_nbm : bool, optional
        classical momentum scheme or a Nesterov momentum scheme

    arg_names : list(str), optional
        special treat weight decay in parameter ends with bias, gamma, and beta
    """
    def __init__(self, learning_rate=1., momentum=0.0,
                 is_nbm=False, rescale_grad=1, **kwargs):
        super(BMUF, self).__init__(rescale_grad=rescale_grad, **kwargs)
        self.lr = learning_rate
        self.momentum = momentum
        self.is_nbm = is_nbm
    ...
    def create_state(self, index, weight):
        ...
        if self.momentum == 0.0:
            ...
            return (zeros(weight.shape, weight.context, dtype=weight.dtype),#mom
                    zeros(weight.shape, weight.context, dtype=weight.dtype) if self.is_nbm else None)#Wt
    ...
    def update(self, index, weight, grad, state):
        ...
        if state:
            ...
            if self.is_nbm:
                Wt[:] += mom
                weight[:] = Wt + self.momentum * mom
            else: #CBM do not need Wt
                weight[:] += mom
        else:
            assert self.momentum == 0.0
            weight[:] += self.lr * grad
...
@register
class MovingAverager(Optimizer):
    """Do Moving Average. Used by stats params. 
    ...
    def __init__(self, momentum, **kwargs):
        super(MovingAverager, self).__init__(**kwargs)
        self.momentum = momentum
    ...
    def update(self, index, weight, grad, state):
        ...
        grad : NDArray
            stats sum of all samples, aggregate like gradients
        ...
        grad = grad * self.rescale_grad
        weight[:] = self.momentum * weight + (1.0 - self.momentum) * grad
...
class MixedOptimizer(Optimizer):
    ...
    Parameters
    ----------
    default_optimizer : (str, kwargs)
       default optimizer
    patterns: list of str
        list of regular expression patterns to match parameter names.
    optimizers: list of (str, kwargs)
        list of optimizer corrosponding to patterns

    """
    def __init__(self, default_optimizer, patterns, optimizers, **kwargs):
        def _create(opt, extra_args=None, **kwargs):
            if extra_args is not None: kwargs.update(extra_args) 
            return self.create_optimizer(opt, **kwargs) if isinstance(opt, str) else opt

        self.default = _create(default_optimizer[0], default_optimizer[1], **kwargs)

        self.optimizers = list(zip([re.compile(p) for p in patterns], 
                                   [_create(o, args, **kwargs) for o, args in optimizers]))

        def find_opt(name):
            for prog, opt in self.optimizers:
                if prog.match(name):
                    return opt
            return self.default

        self.map = {}
        for id, name in kwargs['param_idx2name'].items():
            self.map[id] = find_opt(name)

    def create_state(self, index, weight):
        return self.map[index].create_state(index, weight)

    def update(self, index, weight, grad, state):
        self.map[index].update(index, weight, grad, state)

    def set_lr_mult(self, args_lr_mult):
        self.default.set_lr_mult(args_lr_mult)

        for _, opt in self.optimizers:
            opt.set_lr_mult(args_lr_mult)

    def set_wd_mult(self, args_wd_mult):
        self.default.set_wd_mult(args_wd_mult)

        for _, opt in self.optimizers:
            opt.set_wd_mult(args_wd_mult)
