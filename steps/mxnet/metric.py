...
def ndarray_argmax(pred): #argmax the last dim
    if len(pred.shape) > 2:
        flatten_shape = (numpy.prod(pred.shape[0:-1]), pred.shape[-1])
        pred_label = ndarray.argmax_channel(pred.reshape(flatten_shape))
        return pred_label.reshape(pred.shape[0:-1])
    else:
        return ndarray.argmax_channel(pred)
...
class Accuracy(EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        for i in range(len(labels)):
            pred_label = ndarray_argmax(preds[i]).asnumpy().astype('int32')
            label = labels[i].asnumpy().astype('int32')

            check_label_shapes(label, pred_label)

            self.sum_metric += (pred_label.flat == label.flat).sum()
            self.num_inst += (label.flat >= 0).sum()
