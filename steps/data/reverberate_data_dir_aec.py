...
print(os.getcwd())
data_lib = imp.load_source('dml', './process_env/steps/data/data_dir_manipulation_lib.py')
    ...
    if len(pointsource_noise_list) > 0 and random.random() < pointsource_noise_addition_probability and max_noises_recording >= 1:
        for k in range(random.randint(1, max_noises_recording)):
            ...
            if noise.bg_fg_type == "background":
                noise_rvb_command = """./process_env/bin/wav-reverberate --multi-channel-output=true --impulse-response="{0}" --duration={1}""".format(noise_rir.rir_rspecifier, speech_dur)
                noise_addition_descriptor['start_times'].append(0.002)
                noise_addition_descriptor['snrs'].append(background_snrs.next())
            else:
                # noise_rvb_command = """./process_env/bin/wav-reverberate --impulse-response="{0}" """.format(noise_rir.rir_rspecifier)
...
def GenerateReverberationOpts(room_dict,  # the room dictionary, please refer to MakeRoomDict() for the format
    ...
    if len(rir_iso_noise_list) > 0 and random.random() < isotropic_noise_addition_probability:
        ...
        if len(isotropic_noise.noise_rspecifier.split()) == 1:
            noise_addition_descriptor['noise_io'].append("./process_env/bin/wav-reverberate --multi-channel-output=true --duration={1} {0} - |".format(isotropic_noise.noise_rspecifier, speech_dur))
        else:
            noise_addition_descriptor['noise_io'].append("{0} ./process_env/bin/wav-reverberate --multi-channel-output=true --duration={1} - - |".format(isotropic_noise.noise_rspecifier, speech_dur))
    ...
    for i in range(start_index, num_replicas+1):
        for recording_id in keys:
            ...
            if reverberate_opts == "" or i == 0:
                ...
                wav_corrupted_pipe = "{0} ./process_env/bin/wav-reverberate --multi-channel-output=true --shift-output={1} {2} - - |".format(wav_original_pipe, shift_output, reverberate_opts)
                print(reverberate_opts)
                print(wav_corrupted_pipe)
...
def CreateReverberatedCopy(input_dir,
    ...
    if not os.path.isfile(input_dir + "/reco2dur"):
        ...
        for value in wav_scp.values():
            ...
            if "sox" in value and "speed" in value:
                ...
                break
        data_lib.RunKaldiCommand("./process_env/bin/wav-to-duration --read-entire-file={1} scp:{0}/wav.scp ark,t:{0}/reco2dur".format(input_dir, read_entire_file))
    ...
    data_lib.RunKaldiCommand("./process_env/utils/utt2spk_to_spk2utt.pl <{output_dir}/utt2spk >{output_dir}/spk2utt"
    ...
    data_lib.RunKaldiCommand("./process_env/utils/validate_data_dir.sh --no-feats {output_dir}".format(output_dir = output_dir))
